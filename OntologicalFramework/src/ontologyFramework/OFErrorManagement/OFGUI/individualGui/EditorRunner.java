...
//	private static Boolean doit_objRem = true;
	...
	public EditorRun( final String individualname, final List<List<String>> selectedInfo, final int replacing, final OWLAxiom replacingAxiom) {
			...
			public void mouseReleased(MouseEvent e) {
				//if( doit_replace){
					...
					if( buttonLabel.equals( CONFIRM)){
						...
						JOptionPane.showMessageDialog(a, " Property replaced to Individual", "operation Succesfull", JOptionPane.INFORMATION_MESSAGE);
						caller.dispose();
					}
					//doit_replace = false;
		...
		data_addBnt.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				//if( doit_dataAdd){
					...
					//doit_dataAdd = false;
					JOptionPane a = new JOptionPane();
					JOptionPane.showMessageDialog(a, " data Property added to Individual", "operation Succesfull", JOptionPane.INFORMATION_MESSAGE);
		...
		data_removeBnt.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				//if( doit_dataRem){
					...
					ontoRef.getManager().applyChanges(changes);
					//ClassExcange.getReasoner().flush();
					///doit_dataRem = false;
					JOptionPane a = new JOptionPane();
					JOptionPane.showMessageDialog(a, " data Property removed from Individual", "operation Succesfull", JOptionPane.INFORMATION_MESSAGE);
		...
		obj_addBnt.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				//if( doit_objAdd){
					...
					///doit_objAdd = false;
					JOptionPane a = new JOptionPane();
					JOptionPane.showMessageDialog(a, " object Property added to Individual", "operation Succesfull", JOptionPane.INFORMATION_MESSAGE);
		...
		individual_addBnt.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				OWLReferences ontoRef = OWLReferences.getOWLReferences( ClassExcange.getOntoName());
	...
	private synchronized OWLObjectPropertyAssertionAxiom getObjPropAxiom(){
		...
		OWLNamedIndividual individual = ontoRef.getFactory().getOWLNamedIndividual( individualname, ontoRef.getPm());
		OWLNamedIndividual value = ontoRef.getFactory().getOWLNamedIndividual( propertyValue, ontoRef.getPm());
		OWLObjectProperty hasProperty = ontoRef.getFactory().getOWLObjectProperty( propertyName, ontoRef.getPm());
	...
	private OWLAxiom getDataTypeAxiom(){
		...
		if( data_typeCmbox.getSelectedItem().equals( "integer"))
			isMappedByDataTypeValue =  ontoRef.getFactory()
		...
		OWLAxiom newAxiom = ontoRef.getFactory().getOWLDataPropertyAssertionAxiom(
