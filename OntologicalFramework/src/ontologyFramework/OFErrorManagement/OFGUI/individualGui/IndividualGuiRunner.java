	...
	public synchronized void run() {
		...
		while( !interrupt){// true){
			try {
				if( ! allName.contains( individualname)){
					if ( !allFrameID.contains(frameID)) {
						if( !IndividualRunner.getJustclosed().contains( frameID)){
							...
							ClassExcange.addAllIndividualFrame( a);
				...
				for (Runnable tr : ClassExcange.getAllIndividualFrame()) 
					javax.swing.SwingUtilities.invokeLater( tr);
				
				long done = System.currentTimeMillis() - initialTime; 
				Thread.sleep( ClassExcange.getIndividualFramPeriod() - done);
...
class IndividualRunner implements Runnable {
	...
	protected synchronized void closeWindows() {
		...
		// tabelle
		ClassExcange.removeFromDataTable(dataTable);
		ClassExcange.removeFromObjTable(objTable);
		ClassExcange.removeFromSameIndTable(sameIndTable);
		ClassExcange.removeFromClassTable(classTable);
		...
		// remove this thread
		ClassExcange.removeAllIndividualFrame(thisInstance);
	...
	protected synchronized void setWindows() {
		...
		dataTable = buildTable(panel, splitPane, true, false,
				ClassExcange.dataPropertyTable);
		...
		objTable = buildTable(panel_1, splitPane_1, true, false,
				ClassExcange.objectPropertyTable);
		...
		sameIndTable = buildTable(panel_4, splitPane_2, false, true,
				ClassExcange.classTable);
		classTable = buildTable(panel_3, splitPane_2, true, true,
				ClassExcange.sameIndividualTable);
		...
		saveName.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				ClassExcange.changeVisibilityProgressBar(true);
				ClassExcange.setSavingName(saveName.getText().trim());
				ClassExcange.changeVisibilityProgressBar(false);
			}
		});
	...
	private synchronized ClassTableIndividual buildTable(JPanel panel, JSplitPane splitPane,
		...
		if (table.getTableType() == ClassExcange.dataPropertyTable) {
			ClassExcange.addtoDataTable(table);
		} else if (table.getTableType() == ClassExcange.objectPropertyTable) {
			ClassExcange.addtoObjTable(table);
		} else if (table.getTableType() == ClassExcange.classTable) {
			ClassExcange.addtoClassTable( table);
		} else if (table.getTableType() == ClassExcange.sameIndividualTable) {
			ClassExcange.addtoSameIndTable(table);
		}
