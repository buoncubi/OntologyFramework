...
import ontologyFramework.OFErrorManagement.OFGUI.individualGui.ClassTableIndividual;
...
public class LegendRunner extends JFrame implements ActionListener {
	...
	public LegendRunner() {
		...
		btnPIU.setIcon( ClassExcange.imAddColorIcon);
		...
		btnMENO.setIcon( ClassExcange.imDeleteColorIcon);
		...
		checkBox.setSelected( ClassExcange.isColorMatchSearch());
		checkBox.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(ItemEvent e) {
				...
				ClassExcange.setColorMatchSearch( checkBox.isSelected());
			}
		});
		...
		btnSetFrequencies.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseReleased(MouseEvent e) {
				ClassExcange.setTreePeriod( (Long) treeFreq.getValue());
				ClassExcange.setAllInstancesPeriod( (Long)  logFrq.getValue());
				ClassExcange.setSAVINGPERIOD( (Long) saveFrq.getValue());
				ClassExcange.setIndividualFramPeriod( (Long) indiFreq.getValue());
			}
		});
		...
		btnReset.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseReleased(MouseEvent e) {
				treeFreq.setValue( ClassExcange.getTreePeriod());
				logFrq.setValue( ClassExcange.getAllInstancesPeriod());
				saveFrq.setValue( ClassExcange.getSAVINGPERIOD( ));
				indiFreq.setValue( ClassExcange.getIndividualFramPeriod());
			}
		});
		...
		treeFreq.setModel(new SpinnerNumberModel(new Long( ClassExcange.getTreePeriod()), new Long(10), null, new Long(20)));
		logFrq.setModel(new SpinnerNumberModel(new Long( ClassExcange.getAllInstancesPeriod()), new Long(10), null, new Long(20)));
		saveFrq.setModel(new SpinnerNumberModel(new Long( ClassExcange.getSAVINGPERIOD()), new Long(10), null, new Long(20)));
		indiFreq.setModel(new SpinnerNumberModel(new Long( ClassExcange.getIndividualFramPeriod()), new Long(10), null, new Long(20)));
		...
		lblClass.setIcon( ClassExcange.imClassIcon);

		JLabel lblDataProperty = new JLabel(" Data Property");
		lblDataProperty.setIcon( ClassExcange.imDataPropIcon);

		JLabel lblObjectProperty = new JLabel(" Object Property");
		lblObjectProperty.setIcon( ClassExcange.imObjPropIcon);

		JLabel lblIndividual = new JLabel(" Individual");
		lblIndividual.setIcon( ClassExcange.imIndividualIcon);

		JLabel label = new JLabel(" Class");
		label.setIcon( ClassExcange.imClassInfIcon);

		JLabel label_1 = new JLabel(" Data Property");
		label_1.setIcon( ClassExcange.imDataPropInfIcon);

		JLabel label_2 = new JLabel(" Object Property");
		label_2.setIcon( ClassExcange.imObjPropInfIcon);

		JLabel label_3 = new JLabel(" Individual");
		label_3.setIcon( ClassExcange.imIndividualInfIcon);
		...
		label_4.setIcon( ClassExcange.imClassPredIcon);

		JLabel lblAssertedIndividuals = new JLabel(" asserted individuals");
		lblAssertedIndividuals.setIcon( ClassExcange.imIndividualPredIcon);
	...
	private void showAllItem(){
		colorPane.removeAll();
		for( String key : ClassExcange.getAllColorToFollow().keySet())
			colorPane.add( showNewItem( key, ClassExcange.getAllColorToFollow().get(key)));
		validate();
		repaint();
	}
	...
	private void fireTableChange(){
		List<ClassTableIndividual> data = ClassExcange.getAllDataTable();
		for( ClassTableIndividual ct : data)
			ct.getModel().fireTableDataChanged();
		data = ClassExcange.getAllObjTable();
		for( ClassTableIndividual ct : data)
			ct.getModel().fireTableDataChanged();
		data = ClassExcange.getAllSameIndTable();
		for( ClassTableIndividual ct : data)
			ct.getModel().fireTableDataChanged();
		data = ClassExcange.getAllClassTable();
		for( ClassTableIndividual ct : data)
			ct.getModel().fireTableDataChanged();
		new ClassTree( true);
	}
