...
public class Task5Updater extends OFJobAbstract{

	private static OFDebugLogger logger = new OFDebugLogger( Task5Updater.class, true);

	private static Integer recognitionTimes = 0;

	private static final String COMMONSAVING_filePath = System.getProperty("user.dir") 
			+ "/files/ontologies/SmartHome/evolved/task5/";

	private static String ontoIndividual = "O_Task5DetacherOntology";
	...
	void runJob(JobExecutionContext context) throws JobExecutionException {
		...
		OWLNamedIndividual ontoInd = this.getOWLOntologyRefeferences().getOnlyObjectPropertyB2Individual(
						this.getProcedureIndividualName(),	Task5Dethacer.IMPORTTOONTOLOGY_objProp);
		// get name of Ontology
		String ontoName = this.getOWLOntologyRefeferences().getOnlyDataPropertyB2Individual( ontoInd,
				this.getOWLOntologyRefeferences().getOWLDataProperty( Task5Dethacer.ONTOLOGYNAME_dataProp)).getLiteral();
		//}
		...
		Set<OWLNamedIndividual> inds = task1OntoRef.getIndividualB2Class( Task5Dethacer.VIRTUALSENSOR_className);
		logger.addDebugStrign( this.getProcedureIndividualName() +  " activated virtual sensors: " + inds);
		for( OWLNamedIndividual i : inds){
			if( OWLLibrary.getOWLObjectName( i).equals( Task5Dethacer.RECOGNISED_indName)){
	...
	public static synchronized void cleanOntology(OFBuiltMapInvoker invoker, String ontologyName){
		// reset counter
		Task5Dethacer.setNameCounters( 0);
		Task5Dethacer.setWindowsCount( 0);
		Task5Dethacer.clearTimeline();
		
		// remove and reload ontology
		OWLReferences ontoRefTask = OWLReferences.getOWLReferences("task5Ontology");
