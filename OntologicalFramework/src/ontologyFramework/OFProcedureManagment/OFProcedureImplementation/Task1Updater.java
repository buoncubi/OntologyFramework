...
public class Task1Updater extends OFJobAbstract{

	private static OFDebugLogger logger = new OFDebugLogger( Task1Updater.class, true);

	private static Integer recognitionTimes = 0;

	private static final String COMMONSAVING_filePath = System.getProperty("user.dir") 
			+ "/files/ontologies/SmartHome/evolved/task1/";

	private static String ontoIndividual = "O_Task1DetacherOntology";
	...
	void runJob(JobExecutionContext context) throws JobExecutionException {
		...
		OWLNamedIndividual ontoInd = this.getOWLOntologyRefeferences().getOnlyObjectPropertyB2Individual(
						this.getProcedureIndividualName(),	Task1Dethacer.IMPORTTOONTOLOGY_objProp);
		// get name of Ontology
		String ontoName = this.getOWLOntologyRefeferences().getOnlyDataPropertyB2Individual( ontoInd,
				this.getOWLOntologyRefeferences().getOWLDataProperty( Task1Dethacer.ONTOLOGYNAME_dataProp)).getLiteral();
		//}
		...
		logger.addDebugStrign( this.getProcedureIndividualName() + " importing count: " + Task1Dethacer.importingCount);
		Task1Dethacer.importingCount = 0;
		...
		Set<OWLNamedIndividual> inds = task1OntoRef.getIndividualB2Class( Task1Dethacer.VIRTUALSENSOR_className);
		logger.addDebugStrign( this.getProcedureIndividualName() +  " activated virtual sensors: " + inds);
		for( OWLNamedIndividual i : inds){
			if( OWLLibrary.getOWLObjectName( i).equals( Task1Dethacer.RECOGNISED_indName)){
				...
				logger.addDebugStrign( this.getProcedureIndividualName() +  " activity recognised k!!!!! task1Ontology cleaned, copy available in: " + filePath);
				break;
			}
		}		
	}
	...
	public static synchronized void cleanOntology(OFBuiltMapInvoker invoker, String ontologyName){
		// reset counter
		Task1Dethacer.setNameCounters( 0);
		Task1Dethacer.setWindowsCount( 0);
		Task1Dethacer.clearTimeline();
		
		// clean ontology
		OWLReferences ontoRefTask = OWLReferences.getOWLReferences("task1Ontology");
