...
public class Task8Updater extends OFJobAbstract{

	private static OFDebugLogger logger = new OFDebugLogger( Task8Updater.class, true);

	private static Integer recognitionTimes = 0;

	private static final String COMMONSAVING_filePath = System.getProperty("user.dir") 
			+ "/files/ontologies/SmartHome/evolved/task8/";

	private static String ontoIndividual = "O_Task8DetacherOntology";
	...
	synchronized void runJob(JobExecutionContext context) throws JobExecutionException {
		...
		OWLNamedIndividual ontoInd = this.getOWLOntologyRefeferences().getOnlyObjectPropertyB2Individual(
						this.getProcedureIndividualName(),	Task8Dethacer.IMPORTTOONTOLOGY_objProp);
		// get name of Ontology
		String ontoName = this.getOWLOntologyRefeferences().getOnlyDataPropertyB2Individual( ontoInd,
				this.getOWLOntologyRefeferences().getOWLDataProperty( Task8Dethacer.ONTOLOGYNAME_dataProp)).getLiteral();
		//}
		...
		Set<OWLNamedIndividual> inds = task1OntoRef.getIndividualB2Class( Task8Dethacer.VIRTUALSENSOR_className);
		logger.addDebugStrign( this.getProcedureIndividualName() +  " activated virtual sensors: " + inds);
		for( OWLNamedIndividual i : inds){
			if( OWLLibrary.getOWLObjectName( i).equals( Task8Dethacer.RECOGNISED_indName)){
	...
	public static synchronized void cleanOntology(OFBuiltMapInvoker invoker, String ontologyName){
		// reset counter
		Task8Dethacer.setNameCounters( 0);
		Task8Dethacer.setWindowsCount( 0);
		Task8Dethacer.clearTimeline();
		
		OWLReferences ontoRefTask = OWLReferences.getOWLReferences("task8Ontology");
