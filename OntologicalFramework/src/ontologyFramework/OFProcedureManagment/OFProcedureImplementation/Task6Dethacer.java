...
public class Task6Dethacer extends OFJobAbstract{

	private static OFDebugLogger logger = new OFDebugLogger(Task6Dethacer.class, true);
	...
	//private static final String DOOR07_indName = "S_Door07";
	//private static final String DOOR08_indName = "S_Door08";
	//private static final String DOOR09_indName = "S_Door09";
	private static final String DOOR10_indName = "S_Door10";
	private static final String ITEM01_indName = "S_Item01";
	private static final String ITEM02_indName = "S_Item02";
	private static final String ITEM07_indName = "S_Item07";
	...
	public static final String ITEM1_className = "Item1";
	public static final String ITEM2_className = "Item2";
	public static final String ITEM7_className = "Item7";
	private static final String CONTAINS_doorName = "S_Door";
	...
	public  static final String RECOGNISED_indName = "Vs_MealPrepared";
	...
	static{
	//	indsName.add( DOOR07_indName); property.add( DOOR_dataProp); classes.add( null);
	//	indsName.add( DOOR08_indName); property.add( DOOR_dataProp); classes.add( null);
	//	indsName.add( DOOR09_indName); property.add( DOOR_dataProp); classes.add( null);
		indsName.add( DOOR10_indName); property.add( DOOR_dataProp); classes.add( null);
		indsName.add( ITEM01_indName); property.add( ITEM_dataProp); classes.add( ITEM1_className);
		indsName.add( ITEM02_indName); property.add( ITEM_dataProp); classes.add( ITEM2_className);
		indsName.add( ITEM07_indName); property.add( ITEM_dataProp); classes.add( ITEM7_className);
	...
	synchronized void runJob(JobExecutionContext context) throws JobExecutionException {
		...
		for( int i = 0; i < indsName.size(); i++){
			if (checkState( data_state.get( i), data.get( i))) {
				...
				if ( t != null){
					clock = t;
					if( indsName.get(i).contains( CONTAINS_doorName ))
						importingDoor = true;
				}
		...
		if( dataImported) 
			manageTimeWindows( importingDoor, clock);
	...
	private void manageTimeWindows( boolean doorImported, Long clock){
			
		nameCounter++;
		// update task1Ontology clock only once at the beginning
		if (nameCounter == 1) {
			synchronized(taskOntoRef.getReasoner()){
			OWLLiteral timeStamp = taskOntoRef.getOnlyDataPropertyB2Individual(CLOCK_indName,TIME_dataProp);
		...
		if ((nameCounter > 0) && (doorImported)) { // no interesting data, add empty time windows to use spatial informations
			Long placingTime = Long.valueOf(placingOntoRef.getOnlyDataPropertyB2Individual( abitantName,TIME_dataProp).getLiteral());

			Long margedSize = windowSize;
			Long margedCenter = placingTime;
			...
			//synchronized( taskOntoRef.getReasoner()){	
				...
				/*OWLAxiom axiom = OWLLibrary.setSubClassOf( EMptyTimeWindow_className ,
						w.getClassName(), taskOntoRef);
				List<OWLOntologyChange> changes = OWLLibrary.getAddAxiom(axiom,
						taskOntoRef);
				OWLLibrary.applyChanges(changes, taskOntoRef);
				}
				wasEmptyWindow = true;*/
			//}
		}
	...
	public static synchronized void setWindowsCount(Integer windowsCounter) {
		//synchronized( windowsCounter){
			Task6Dethacer.windowsCount = windowsCounter;
		//}
	}
