...
public class Task7Updater extends OFJobAbstract {

	private static OFDebugLogger logger = new OFDebugLogger(Task7Updater.class,
			true);

	private static Integer recognitionTimes = 0;

	private static final String COMMONSAVING_filePath = System.getProperty("user.dir") 
			+ "/files/ontologies/SmartHome/evolved/task7/";

	private static String ontoIndividual = "O_Task7DetacherOntology";
	...
	void runJob(JobExecutionContext context) throws JobExecutionException {
		...
		//synchronized (this.getOWLOntologyRefeferences().getReasoner()) {
			OWLNamedIndividual ontoInd = this.getOWLOntologyRefeferences()
					.getOnlyObjectPropertyB2Individual(	this.getProcedureIndividualName(),DataImporter.IMPORTTOONTOLOGY_objProp);
		...
		//synchronized (task1OntoRef.getReasoner()) {
			...
			Set<OWLNamedIndividual> performingWindows = task1OntoRef
					.getObjectPropertyB2Individual(PERFORMINGVIRTUALSENSOR_indName, BELONGTW_objProp);
			...
			for (OWLNamedIndividual performingInd : performingWindows) {
				performingTime += Long.valueOf(task1OntoRef.getOnlyDataPropertyB2Individual(performingInd,
								siseProp).getLiteral());
			}
			...
			Set<OWLNamedIndividual> inds = task1OntoRef.getIndividualB2Class(
					Task7Dethacer.VIRTUALSENSOR_className);
			...
			for (OWLNamedIndividual i : inds) {
				if (OWLLibrary.getOWLObjectName(i).equals(
						Task7Dethacer.RECOGNISED_indName)) {
					// get test name from dataset file
					OWLNamedIndividual fileInd = this.getOWLOntologyRefeferences()
							.getOnlyObjectPropertyB2Individual(PROCEDUREIMPORTER_indNAme,IMPORTSFROMFILE_objProp);
					String completePath = this.getOWLOntologyRefeferences()
							.getOnlyDataPropertyB2Individual( fileInd,
	...
	public static synchronized void cleanOntology(OFBuiltMapInvoker invoker,
			String ontologyName) {
		// reset counter
		Task7Dethacer.setNameCounters(0);
		Task7Dethacer.setWindowsCount(0);
		Task7Dethacer.clearTimeline();

		// remove and reload ontology
		OWLReferences ontoRefTask = OWLReferences.getOWLReferences("task7Ontology");
